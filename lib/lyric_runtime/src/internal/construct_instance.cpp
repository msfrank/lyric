...
tempo_utils::Status
lyric_runtime::internal::construct_instance(
    tu_uint32 address,
    tu_uint8 flags,
    ...
    InterpreterState *state)
{
    ...
    // resolve the instance descriptor
    ...
    auto *vtable = segmentManager->resolveInstanceVirtualTable(receiver, status);
    TU_RETURN_IF_NOT_OK (status);
    ...
    if (numArguments != 0)
        return InterpreterStatus::forCondition(
            InterpreterCondition::kRuntimeInvariant, "unexpected arguments for instance ctor");
    if (numLexicals != 0)
        return InterpreterStatus::forCondition(
            InterpreterCondition::kRuntimeInvariant, "unexpected lexicals for instance ctor");
    ...
    // store the instance
    if (!segmentManager->storeInstance(address, ref, currentCoro, status))
        return status;

    TU_LOG_V << "constructed instance " << ref;
