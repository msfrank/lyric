...
lyric_parser::internal::ModuleDefinstanceOps::enterDefinstanceStatement(ModuleParser::DefinstanceStatementContext *ctx)
{
    ...
    // set the instance super type if specified
    if (ctx->instanceBase()) {
        auto *superTypeNode = make_Type_node(state, ctx->instanceBase()->assignableType());
        TU_RAISE_IF_NOT_OK (definstanceNode->putAttr(kLyricAstTypeOffset, superTypeNode));
    }
...
void
lyric_parser::internal::ModuleDefinstanceOps::enterInstanceDecl(ModuleParser::InstanceDeclContext *ctx)
{
    tempo_tracing::EnterScope scope("lyric_parser::internal::ModuleDefinstanceOps::enterInstanceDecl");
}

void
lyric_parser::internal::ModuleDefinstanceOps::exitInstanceDecl(ModuleParser::InstanceDeclContext *ctx)
{
    tempo_tracing::ExitScope scope;
    ...
    // append decl node to definstance
    TU_RAISE_IF_NOT_OK (definstanceNode->appendChild(declNode));
}
...
lyric_parser::internal::ModuleDefinstanceOps::exitDefinstanceStatement(ModuleParser::DefinstanceStatementContext *ctx)
{
    ...
    // set the visibility
    TU_RAISE_IF_NOT_OK (definstanceNode->putAttr(kLyricAstIsHidden, isHidden));

    // set the abstract flag if it is not set already
    if (!definstanceNode->hasAttr(kLyricAstIsAbstract)) {
        TU_RAISE_IF_NOT_OK (definstanceNode->putAttr(kLyricAstIsAbstract, false));
    }

    // set the derive type if it is not set already
    if (!definstanceNode->hasAttr(kLyricAstDeriveType)) {
        TU_RAISE_IF_NOT_OK (definstanceNode->putAttr(kLyricAstDeriveType, DeriveType::Any));
    }
}