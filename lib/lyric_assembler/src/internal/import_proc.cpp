...
get_variable_url(std::string_view name, ImportProcData &data)
{
...
static tempo_utils::Status
apply_load(const lyric_object::OpCell &op, ImportProcData &data)
{
    ...
    switch (operands.flags) {
        ...
        case lyric_object::LOAD_ARGUMENT: {
            ...
            auto argumentSymbol = std::make_unique<lyric_assembler::ArgumentVariable>(
                argumentUrl, operands.address, data.state);
        ...
        case lyric_object::LOAD_LEXICAL: {
            ...
            auto lexicalSymbol = std::make_unique<lyric_assembler::LexicalVariable>(
                lexicalUrl, operands.address, data.state);
            auto *symbolCache = data.state->symbolCache();
        ...
        case lyric_object::LOAD_LOCAL: {
            ...
            auto localSymbol = std::make_unique<lyric_assembler::LocalVariable>(
                localUrl, /* isHidden= */ true, operands.address, data.state);
            auto *symbolCache = data.state->symbolCache();
...
static tempo_utils::Status
apply_descriptor(const lyric_object::OpCell &op, ImportProcData &data)
{
    const auto &operands = op.operands.flags_u8_address_u32;
...
static tempo_utils::Status
apply_store(const lyric_object::OpCell &op, ImportProcData &data)
{
    const auto &operands = op.operands.flags_u8_address_u32;
    switch (operands.flags) {
...
static tempo_utils::Status
apply_jump(const lyric_object::OpCell &op, ImportProcData &data)
{
    ...
    ObjectState *state)
{
    ...
    absl::flat_hash_map<tu_uint32,JumpLabel> jumpLabels;
    ...
    procHandle = std::move(importedProc);
    return {};
}
