    ...
    lyric_typing::TypeSystem *typeSystem)
{
    ...
    auto *superCtorCall = superClass->getCtor(lyric_object::kCtorSpecialSymbol);
    if (superCtorCall->numParameters() > 0)
        return CompilerStatus::forCondition(CompilerCondition::kCompilerInvariant,
            "super class {} is not default-constructable", superCtorCall->getSymbolUrl().toString());
    ...
    lyric_typing::TypeSystem *typeSystem)
{
    ...
    TU_LOG_V << "declared impl " << impl.implType << " for " << classSymbol->getSymbolUrl();
