    ...
    BeforeContext &ctx)
{
    ...
    for (auto &declNode : declNodes) {
        Stub stub;
        TU_ASSIGN_OR_RETURN (stub, declare_struct_stub(
            declNode, m_defstruct.structSymbol, typeSystem));
        m_defstruct.stubs[declNode] = stub;
    }
    ...
    for (auto &globalNode : globalNodes) {
        TU_RETURN_IF_NOT_OK (declare_global_block(globalNode, &m_defstruct.global, typeSystem));
    }
    ...
    DecideContext &ctx)
{
    ...
    switch (astId) {
        ...
        case lyric_schema::LyricAstId::Decl: {
            auto stub = m_defstruct->stubs.at(node);
        ...
        case lyric_schema::LyricAstId::Global: {
            auto handler = std::make_unique<GlobalHandler>(&m_defstruct->global, block, driver);
            ctx.setGrouping(std::move(handler));
            return {};
        }
        default:
