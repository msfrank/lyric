...
lyric_packaging::PackageWriter::PackageWriter(const PackageWriterOptions &options)
    : m_options(options),
...
tempo_utils::Status
lyric_packaging::PackageWriter::configure()
{
...
bool
lyric_packaging::PackageWriter::hasEntry(const EntryPath &path) const
{
...
bool
lyric_packaging::PackageWriter::hasEntry(EntryAddress parentDirectory, std::string_view name) const
{
...
lyric_packaging::PackageWriter::getEntry(const EntryPath &path) const
{
    TU_ASSERT (path.isValid());
...
tempo_utils::Result<lyric_packaging::EntryAddress>
lyric_packaging::PackageWriter::makeDirectory(const EntryPath &path, bool createIntermediate)
{
    TU_ASSERT (path.isValid());
...
tempo_utils::Result<lyric_packaging::EntryAddress>
lyric_packaging::PackageWriter::makeDirectory(EntryAddress parentDirectory, std::string_view name)
{
...
tempo_utils::Result<lyric_packaging::EntryAddress>
lyric_packaging::PackageWriter::putFile(
...
tempo_utils::Result<lyric_packaging::EntryAddress>
lyric_packaging::PackageWriter::linkToTarget(const EntryPath &path, EntryAddress target)
{
    TU_ASSERT (path.isValid());
...
lyric_packaging::PackageWriter::toBytes() const
{
    ...
    appender.appendBytes(std::string_view(lpm1::ManifestIdentifier(), strlen(lpm1::ManifestIdentifier())));
