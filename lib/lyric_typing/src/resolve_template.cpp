    ...
    lyric_assembler::ObjectState *state)
{
    ...
    if (!walker.isClass(lyric_schema::kLyricAstGenericClass))
        return TypingStatus::forCondition(TypingCondition::kTypingInvariant,
            "failed to parse template; unexpected parser node");
    ...
    for (int i = 0; i < walker.numChildren(); i++) {
        ...
        if (!placeholder.isClass(lyric_schema::kLyricAstPlaceholderClass))
            return TypingStatus::forCondition(TypingCondition::kTypingInvariant,
                "failed to parse template placeholder; unexpected parser node");
        ...
        if (placeholder.numChildren() > 0) {
            ...
            if (!constraint.isClass(lyric_schema::kLyricAstConstraintClass))
                return TypingStatus::forCondition(TypingCondition::kTypingInvariant,
                    "failed to parse template constraint; unexpected parser node");
            ...
            TypeSpec assignable;
            TU_ASSIGN_OR_RETURN (assignable, parse_assignable(block, typeNode));
