...
tempo_utils::Status
lyric_analyzer::InstanceAnalyzerContext::declareMember(const lyric_parser::ArchetypeNode *node)
{
...
tempo_utils::Status
lyric_analyzer::InstanceAnalyzerContext::declareStub(const lyric_parser::ArchetypeNode *node)
{
    std::string identifier;
    TU_RETURN_IF_NOT_OK (node->parseAttr(lyric_parser::kLyricAstIdentifier, identifier));
    ...
    TU_ASSIGN_OR_RETURN (actionSymbol, m_instanceSymbol->declareStub(identifier, isHidden));
    ...
    TU_LOG_V << "declared stub " << actionSymbol->getSymbolUrl() << " for " << m_instanceSymbol->getSymbolUrl();
