...
lyric_analyzer::RootAnalyzerContext::RootAnalyzerContext(
...
lyric_analyzer::RootAnalyzerContext::getBlock() const
{
    return m_root->rootBlock();
}

tempo_utils::Status
lyric_analyzer::RootAnalyzerContext::enter(
    ...
    lyric_rewriter::VisitorContext &ctx)
{
    ...
    switch (astId) {
        case lyric_schema::LyricAstId::Entry:
            return m_driver->pushEntry(node, getBlock());
...
tempo_utils::Status
lyric_analyzer::RootAnalyzerContext::exit(
    const lyric_parser::ArchetypeState *state,
    const lyric_parser::ArchetypeNode *node,
