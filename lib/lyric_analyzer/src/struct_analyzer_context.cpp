...
lyric_analyzer::StructAnalyzerContext::declareMethod(const lyric_parser::ArchetypeNode *node)
{
    ...
    TU_LOG_V << "declared method " << callSymbol->getSymbolUrl() << " for " << m_structSymbol->getSymbolUrl();
...
tempo_utils::Status
lyric_analyzer::StructAnalyzerContext::declareStub(const lyric_parser::ArchetypeNode *node)
{
    std::string identifier;
    TU_RETURN_IF_NOT_OK (node->parseAttr(lyric_parser::kLyricAstIdentifier, identifier));
    ...
    TU_ASSIGN_OR_RETURN (actionSymbol, m_structSymbol->declareStub(identifier, isHidden));
    ...
    TU_LOG_V << "declared stub " << actionSymbol->getSymbolUrl() << " for " << m_structSymbol->getSymbolUrl();
...
lyric_analyzer::StructAnalyzerContext::declareImpl(const lyric_parser::ArchetypeNode *node)
{
    ...
    TU_LOG_V << "declared impl " << implType << " on " << m_structSymbol->getSymbolUrl();
