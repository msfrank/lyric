    ...
    VisitorContext &ctx)
{
    ...
    TU_LOG_VV << "enter " << resource->getName() << " node with child index " << ctx.childIndex();

    if (astId == lyric_schema::LyricAstId::MacroList) {
        TU_LOG_VV << "skipping macro list children";
    ...
    const VisitorContext &ctx)
{
    ...
    TU_LOG_VV << "exit " << resource->getName() << " node with child index " << ctx.childIndex();
