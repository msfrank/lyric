...
lyric_rewriter::AstDefconceptVisitor::AstDefconceptVisitor(
...
tempo_utils::Status
lyric_rewriter::AstDefconceptVisitor::enter(lyric_parser::ArchetypeNode *node, VisitorContext &ctx)
{
    ...
    for (int i = 0; i < node->numChildren(); i++) {
        ...
        switch (childId) {
            case lyric_schema::LyricAstId::Decl:
                actionNodes.emplace_back(child, i);
                break;
...
tempo_utils::Status
lyric_rewriter::AstDefconceptVisitor::exit(lyric_parser::ArchetypeNode *node, const VisitorContext &ctx)
{
