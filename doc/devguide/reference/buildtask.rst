...
Compiles the source code for a module into a loadable object. compile_object declares a dependency
on parse_archetype and symbolize_linkage so that the module archetype and linkage artifacts are
...
1. In phase one, the module linkage is scanned for relative imports. For each import a new
   analyze_outline dependency is declared. If the module linkage requires a plugin, then a
...
1. In phase two, the module object is compiled from the module archetype, the module
   outlines generated by the analyze_outline dependencies, and the optional plugin.
